...
#     file.write(base64.b64decode(response["metad   ata"]))

auth = requests.auth.HTTPBasicAuth("shushakishkish", "kekg123gg667")

# response = requests.post("http://www.d1ffic00lt.com/app/api/v1.0/db/detections/", json=json_example, auth=auth)
# print(response)
# print(response.json())
response = requests.delete("http://www.d1ffic00lt.com/app/api/v1.0/db/detections/10", auth=auth)
print(response)
# response = response.json()
# with open(response["filename"], "+wb") as file:
